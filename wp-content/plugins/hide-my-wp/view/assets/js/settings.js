(function ($) {
    ...
    $.fn.hmwp_fixSettings = function (name, value) {
        ...
        $.post(
            ...
            $this.hmwp_loading(false);
            $('body').prepend('<div class="text-center hmwp_notice hmwp_notice_fixed danger" role="alert">Ajax Error.</div>');
    ...
    $.fn.hmwp_securityCheckListen = function () {
        ...
        $this.find('form.hmwp_securityexclude_form').on('submit', function () {
            var $form = $(this);
        ...
        $this.find('form#hmwp_securitycheck').on('submit', function () {
            var $form = $(this);
        ...
        $this.find('form#hmwp_resetexclude').on('submit', function () {
            var $form = $(this);
            $.post(
                ajaxurl,
                $form.serialize()
    ...
    $.fn.hmwp_settingsListen = function () {
        ...
        $this.find(".hmwp_nav_item").on('click', function ( ev ) {
            ev.preventDefault();
        ...
        $('button.hmwp_modal').on('click', function () {
            var $button = $(this);

            if($button.data("remote")){
                $($button.data("target") + ' .modal-body').attr('src',$button.data("remote"));

                $($button.data("target")).on('hidden.bs.modal', function () {
        ...
        $this.find("input.switch").not('.nopopup').change(function () {
        ...
        $this.find("input").not('.nopopup').change(function () {
            //set settings as unsaved to prevent browsing our
            unsaved = true;
        });

        $this.find("input.switch").each(function () {
            if (!$(this).prop('checked')){
                if($('div.' + $(this).attr('name')).length) {
                    $('div.' + $(this).attr('name')).hide();
                }
        ...
        //hide WP Core Paths
        $this.find("input[name=hmwp_hide_oldpaths_plugins].switch").change(function () {
            if ($(this).prop('checked')) {
                $this.find('input[name=hmwp_hide_oldpaths]').prop("checked", true);
            }
        });
        $this.find("input[name=hmwp_hide_oldpaths_themes].switch").change(function () {
            if ($(this).prop('checked')) {
                $this.find('input[name=hmwp_hide_oldpaths]').prop("checked", true);
            }
        });
        ...
        $("#hmw_plugins_mapping_new").on('change', function ( ev ) {
        ...
        $("#hmw_themes_mapping_new").on('change', function ( ev ) {
            var $name = $(this).find(":selected").text();
            var $value = $(this).find(":selected").val();
        ...
        //Header Security
        $("#hmwp_security_headers_new").on('change', function ( ev ) {
            var $name = $(this).find(":selected").text();
            var $value = $(this).find(":selected").val();
            var $div = $('div.' + $name);
        ...
        $this.find("button.brute_use_math").on('click', function () {
        ...
        $this.find("button.brute_use_captcha").on('click', function () {
        ...
        $this.find("button.brute_use_captcha_v3").on('click', function () {
            $this.find('input[name=brute_use_captcha]').val(0);
            $this.find('input[name=brute_use_math]').val(0);
        ...
        $this.find('#hmwp_blockedips_form').on('submit',function () {
        ...
        $this.find('.ajax_submit input').on('change',function () {
